...
final_data_clean <- final_data %>%
  mutate(
    ...
    clean_treatment = case_when(
      formative_regime_change == 1 ~ 1,                    # Treated: experienced change during formative years
...
# Check your new sample sizes
table(final_data_clean$clean_treatment)
```

```{r}
# Method 1: Country counts by treatment type
country_breakdown <- final_data_clean %>%
  group_by(treatment_type, country) %>%
  summarise(n_individuals = n(), .groups = "drop") %>%
  arrange(treatment_type, desc(n_individuals))

print("Country breakdown by treatment type:")
print(country_breakdown)

# Method 2: Summary table - countries per treatment type
...
# Method 3: Wide format showing individuals per country per treatment type
country_wide <- final_data_clean %>%
  count(country, treatment_type) %>%
  pivot_wider(names_from = treatment_type, values_from = n, values_fill = 0) %>%
  arrange(desc(Control + Democratization + Autocratization))

print("Countries with individual counts by treatment type:")
print(country_wide)

# Method 4: Focus on treated countries - which countries had which type of changes
...
# Method 5: Control countries (stable countries)
control_countries <- final_data_clean %>%
  filter(treatment_type == "Control") %>%
  count(country, sort = TRUE)

print("Control group countries (stable countries):")
