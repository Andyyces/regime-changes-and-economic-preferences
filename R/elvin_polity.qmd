...
create_country_regime_data_polity <- function(polity_data,
  ...
  # Replace special polity codes (66, 77, 88) with NA
  polity_data <- polity_data %>%
    mutate(
      polity = ifelse(polity %in% c(66, 77, 88), NA, polity)
    )
  
  # Sort and compute regime change using polity scores
  country_regime_changes <- polity_data %>%
    arrange(country, year) %>%
    group_by(country) %>%
    mutate(
      polity_lag = lag(polity, lag_years),
      polity_diff = polity - polity_lag,
      
      # Identify regime change: 2 = democratization, 1 = autocratization, 0 = no change
      regime_change = case_when(
        !is.na(polity_diff) & polity_diff >= democratization_threshold ~ 2,
        !is.na(polity_diff) & polity_diff <= autocratization_threshold ~ 1,
        TRUE ~ 0
      ),
    ...
    ungroup() %>%
    # 👉 Filtering only years after 1920
    filter(year >= 1920) %>%
    select(country, scode, year, polity, polity_lag, polity_diff, regime_change, treatment_factor)
...
merge_polity_changes_with_individuals <- function(gps_data, polity_regime_data) {
  ...
  # 2. Merge by isocode (GPS) and scode (Polity)
  ...
  individual_summary <- merged_data %>%
    group_by(id_gallup) %>%
    summarize(
      across(c(age,  isocode, region, patience, risktaking, posrecip, negrecip, altruism, trust, subj_math_skills, gender, birth_year, year_3, year_adult), 
             first, .names = "{.col}"),
...
calculate_avg_gdppc <- function(final_data, gdp_data) {
  ...
  for (i in 1:nrow(final_data)) {
    ...
    # Filter GDP data for this country and period
