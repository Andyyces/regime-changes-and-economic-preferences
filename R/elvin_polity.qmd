...
create_country_regime_data_polity <- function(polity_data,
  ...
  # 1. Filter for specific countries if provided
  if (!is.null(country_filter)) {
    polity_data <- polity_data %>%
      filter(country %in% country_filter)
  }

  # 2. Replace special polity2 codes with NA
  polity_data <- polity_data %>%
    mutate(polity2 = ifelse(polity2 %in% c(66, 77, 88), NA, polity2))

  # 3. Classify regime types
  polity_data <- polity_data %>%
    mutate(
      regime_type = case_when(
        is.na(polity2) ~ NA_character_,
        polity2 >= 6 ~ "Democracy",
        polity2 <= 5 ~ "Authoritarian"
      )
    )

  # 4. Compute regime change using lag (no suppression of repeated changes)
  country_regime_changes <- polity_data %>%
    ...
    mutate(
      ...
      regime_type_lag = if_else(
        year >= min_year + lag_years,
        lag(regime_type, lag_years),
        NA_character_
      ),

      regime_change_raw = case_when(
        !is.na(regime_type) & !is.na(regime_type_lag) & regime_type != regime_type_lag ~ 1,
        !is.na(regime_type) & !is.na(regime_type_lag) & regime_type == regime_type_lag ~ 0,
        TRUE ~ NA_real_
      ),

      # Final regime_change: 2 = Democratization, 1 = Autocratization, 0 = No change
      regime_change = case_when(
        regime_change_raw == 1 & regime_type_lag == "Authoritarian" & regime_type == "Democracy" ~ 2,
        regime_change_raw == 1 & regime_type_lag == "Democracy" & regime_type == "Authoritarian" ~ 1,
        TRUE ~ 0
      ),
    ...
    ungroup() %>%
    filter(year >= 1920) %>%
    select(country, scode, isocode, ccode, year,
           polity2, regime_type, regime_type_lag,
