...
The goal is to asses the quality of a TWFE estimator by examining the extent to which it p
...
```{r}
create_density_plots_treated <- function(data) {
  ...
  continuous_vars <- c("avg_gdppc_formative", "avg_libdem_formative")
  plot_list <- list()
  
  for(var in continuous_vars) {
    p <- treated_data %>%
      filter(!is.na(!!sym(var))) %>%
      ggplot(aes(x = !!sym(var), fill = treatment_type)) +
      geom_density(alpha = 0.6) +
      scale_fill_manual(values = c("Democratization" = "#2166ac", "Autocratization" = "#d73027")) +
      labs(title = paste("Distribution of", var, "(Treated Groups Only)"),
           x = var,
           y = "Density",
           fill = "Treatment Type") +
      theme_minimal() +
...
density_plots_treated <- create_density_plots_treated(final_data_gdp)

# Display plots
grid.arrange(grobs = density_plots_treated, ncol = 2)

```

```{r}
calculate_smd_treated <- function(data) {
  ...
  for(var in balance_vars) {
    ...
    if(length(autocrat_vals) > 1 & length(democrat_vals) > 1) {
      pooled_sd <- sqrt((var(autocrat_vals) + var(democrat_vals)) / 2)
      smd <- (mean(democrat_vals) - mean(autocrat_vals)) / pooled_sd
...
smd_data_treated <- calculate_smd_treated(final_data_gdp)

# Love plot for treated groups
smd_data_treated %>%
  mutate(
    variable = factor(variable, levels = rev(variable)),
    balanced = abs(smd) < 0.1
  ) %>%
  ggplot(aes(x = smd, y = variable, color = balanced)) +
  geom_point(size = 3) +
  geom_vline(xintercept = c(-0.1, 0, 0.1), linetype = c("dashed", "solid", "dashed")) +
  scale_color_manual(values = c("TRUE" = "#2166ac", "FALSE" = "#d73027"),
                     labels = c("Unbalanced (|SMD| > 0.1)", "Balanced (|SMD| ≤ 0.1)")) +
  labs(title = "Standardized Mean Differences: Democratization vs Autocratization",
       subtitle = "Balance assessment within treated groups only",
