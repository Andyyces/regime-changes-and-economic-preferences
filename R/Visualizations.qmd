...
for (cont in continents) {
  ...
  continent_plot <- ggplot(continent_data, 
    ...
    scale_fill_manual(values = c("No change" = "gray", 
                                "Autocratization" = "#8B0000", 
                                "Democratization" = "#00008B"),
                      name = "Regime Change Type") +
    theme_minimal() +
...
saveRDS(all_countries_plot, here("Output","figures", "regime_change_visualization.rds"))

```

```{r}
# Creating A4-sized visualization with ORIGINAL orientation (which works better)
...
# Creating the visualization with A4 dimensions - ORIGINAL ORIENTATION
all_countries_plot <- ggplot(country_data_ordered,
                           aes(x = year, y = country_text_id, fill = treatment_factor)) +
  geom_tile(color = "white", size = 0.05) +
  scale_fill_manual(values = c("No change" = "gray95",
                              "Autocratization" = "#B80000",
