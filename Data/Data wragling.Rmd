...
gps_sub <- GPS_indiv %>%
  ...
  select(id_gallup, country, region, patience, risktaking, posrecip, negrecip, altruism, trust, subj_math_skills, gender, birth_year)
...
gps_sub <- gps_sub %>%
  mutate(year_adult = birth_year + 18) # Maybe we should base this decision (whether 18/25 any different age) on some literature? I will take a look.

#merging data
merged_data <- gps_sub %>%
  left_join(vdem_with_regimes, 
            by = c("country" = "country_name", "year_adult" = "year"))
...
final_data <- generate_formative_regime_change(vdem_data, gps_sub, threshold = 0.03)
