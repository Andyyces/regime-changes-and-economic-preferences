...
The approach above assigns the following indviduals to be a part the control group:
...
The contaminated control group could compromise the integrity of our study, because these individuals might have indirect exposure effects. In order to avoid this, we have decide to delete these observations.

Our **control group** on the other hand consists of **individuals from countries, which never experienced a regime change**.
...
country_summary %>%
  kbl(caption = "Summary of Countries by Regime Change Type",
      format = "latex",
      booktabs = TRUE,
      linesep = "") %>%
...
-   **Residual selection bias:**\
    ...
    **Mitigation:** We include **control variables**, such as average GDP per capita during formative years (`gdppc_2012`) and **subjective math skills**, to account for socioeconomic background and individual cognitive differences.
