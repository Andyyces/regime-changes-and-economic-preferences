...
```{r}
#| echo: false
# Function to create improved formatted tables for the paper
create_formatted_table <- function(data, vars, title, caption = NULL) {
  # Reshape the data as in your original function
  table_data <- data %>%
    pivot_longer(
      cols = contains(paste0(vars, "_")),
      names_to = c("variable", "stat"),
  ...
  # Format using kableExtra for better PDF output
...
# Creating improved tables for preferences and controls
preferences_table <- create_formatted_table(
  summary_table,
  c("trust", "patience", "risktaking", "posrecip", "negrecip", "altruism"),
  "Economic Preferences",
