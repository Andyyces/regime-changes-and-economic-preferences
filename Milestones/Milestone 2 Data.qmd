...
-   Vdem (@coppedge_v-dem_2025) dataset comes also as an R package: `r vdem <- vdemdata::vdem`. This dataset contains various democracy indicators for XXX countries starting of year XXX. We will be using the Liberal democracy index which combines many of these indicators into a single number for each country/year.

-   Global Preference Survey (@falk_global_2018) dataset was downloaded as a ZIP file from the [following website](https://gps.iza.org/downloads). This dataset contains information about economic preferences of 80337 individuals from XX countries. The survey was conducted between 2012 and 2013.
...
Our sample period is defined by the birth years of the indviduals, captured in the GPS dataset. Each study unit (=individual) appears once. We have data of 80337 individuals.
...
For completeness, we include **summary statistics** for all 6 economic preferences in Table XX.
...
Our main key variable is **`TRUST.`** The graph below depicts the average **`TRUST`** values grouped by country in *2012* and *2013*.
...
world_data <- left_join(world, country_trust, by = c("name" = "country"))
...
world <- ne_countries(scale = "medium", returnclass = "sf")
world_data <- left_join(world, country_patience, by = c("name" = "country"))
...
ggplot(final_data_gdp, aes(x = formative_regime_change, y = trust)) +
  ...
  labs(
    x = "Regime Change experienced during formative years",
    y = "Trust",
