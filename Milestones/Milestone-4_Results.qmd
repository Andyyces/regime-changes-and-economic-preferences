...
Our results validate our concern about pooling transitions. The findings indicate that **democratization** has a statistically significant positive effect on **patience** ($β=$ `r round(hetero_models[["Patience"]]$coefficients["democratization"], 3)`, p \< 0.01), suggesting that individuals exposed to transitions toward democracy during formative years tend to be more patient. Meanwhile, autocratization shows no significant effect on patience. This important assymetry was missed by our [Model of Combined Effects]. **Both transitions** are associated with **negative reciprocity**, with democratization showing $β=$ `r round(hetero_models[["Negative reciprocity"]]$coefficients["democratization"], 3)`, p \< 0.05, and autocratization showing $β=$ `r round(hetero_models[["Negative reciprocity"]]$coefficients["autocratization"], 3)`, p \< 0.05.
...
-   The **logarithm of the average GDP per capita during the formative period** – proxies for the general level of economic development and material conditions that shape individuals' economic epportunities and constraints;
