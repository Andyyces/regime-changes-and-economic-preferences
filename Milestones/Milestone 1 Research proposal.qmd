...
subtitle: "Milestone 1: Research Proposal"
...
# Filter for 2018 and select relevant columns
vdem_2018 <- vdem_data %>%
  filter(year == 2018) %>%
  select(country_name, country_text_id, v2x_libdem) %>%
  rename(iso_a3 = country_text_id)

# Merge with world map data
world_data <- left_join(world, vdem_2018, by = "iso_a3")
...
world_map <- ggplot(data = world_data) +
  geom_sf(aes(fill = v2x_libdem), color = "gray30", size = 0.1) +
...
png(here("Output", "figures", "world_map.png"), width = 1000, height = 600)
plot(world_map)
dev.off()
...
df <- readRDS(here("Input" , "clean" , "final_data_gdp.rds"))
df  

# creating ages groups
df <- df %>%
  mutate(age_group = cut(age,
                         breaks = c(3, 18, 30, 45, 60, 70, 80, 95, Inf),
                         labels = c("3-18", "19-30", "31-44", "45-60",
                                    "55-64", "65-74", "61-80", "81+"),
                         right = TRUE))

# calculation of ages percentage
age_dist <- df %>%
  count(age_group) %>%
  mutate(percentage = round(n / sum(n) * 100, 1))

#Histogram
histogram_age_distrubition <- ggplot(age_dist, aes(x = age_group, y = percentage)) +
  geom_bar(stat = "identity", fill = "#00A88F") +
  ...
  theme_minimal(base_size = 14) +
  ylim(0, max(age_dist$percentage) + 5)

png(here("Output", "figures", "histogram_age_distrubition.png"), width = 1000, height = 600)
plot(histogram_age_distrubition)
dev.off()
```
...
We will be using TWFE approach to examine the relationship between regime changes(=treatment) and economic preferences on individual level across multiple countries and time periods. This will allow us to compare the dynamics of economic preferences of different, using subjects from countries without regime changes as control groups.
...
The non-experiment treatment is exposure to regime changes, particularly democratization or autocratization. A treated individual would be someone who lived through a regime change in their country. But the treatment could vary based on:
...
Country-level: Avergae GDP per capita during formative years (is currently not included in our available data, we would have to add)
