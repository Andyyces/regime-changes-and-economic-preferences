...
These categories are based on V-Dem's extensive dataset, which includes expert-coded indicators on political institutions, rights, and governance.
...
# LDE modellərini yüklə (quş olanlar)
...
```{r}
 #Əgər model_names obyektiniz varsa onu istifadə edin, əks halda sadə adlar yaradaq:
# Başlıq üçün model_names obyektindən istifadə edirik

preference_vars_1 <- c("Patience", "Risk taking", "Positive reciprocity", "Negative reciprocity", "Altruism", "Trust")

titles <- paste("Preference:", preference_vars_1)

# Bütün nəticə cədvəllərini siyahı kimi yaradın
tables <- lapply(seq_along(hetero_models_v2), function(i) {
  msummary(
    list(
      "Combined (Dem. + Aut.)" = hetero_models_v2[[i]],
      "Democratization only"   = hetero_models_democ_v2[[i]],
      "Autocratization only"   = hetero_models_autoc_v2[[i]]
    ),
    stars = TRUE,
    coef_rename = c(
      "democratization" = "Democratization",
...
# LDE-based modelləri yükləyirik (quş olanlar)
...
```{r}
# Cədvəlləri siyahı şəklində yarat
...
# Başlıqlar: preference adları
preference_vars_1 <- c("Patience", "Risk taking", "Positive reciprocity", "Negative reciprocity", "Altruism", "Trust")
titles <- paste("Preference:", preference_vars_1)

# Bütün nəticə cədvəllərini siyahı kimi yaradın
tables_controls <- lapply(seq_along(hetero_models_controls_v2), function(i) {
  msummary(
    list(
      "Combined (Dem. + Aut.)" = hetero_models_controls_v2[[i]],
      "Democratization only"   = hetero_models_democ_controls_v2[[i]],
      "Autocratization only"   = hetero_models_autoc_controls_v2[[i]]
    ),
    stars = TRUE,
    coef_rename = c(
      "democratization" = "Democratization",
